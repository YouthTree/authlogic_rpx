    ...
    private
      ...
      def validate_by_rpx
        ...
        if @rpx_data.nil? || @rpx_data['profile'].nil?
          errors.add(:base, "Authentication failed. Please try again.")
          return false
        end
        ...
        if !attempted_record
          if auto_register?
            ...
            errors.add(:base, "We did not find any accounts with that login. Enter your details and create an account.")
            return false
          end
        else
